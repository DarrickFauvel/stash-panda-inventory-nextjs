...
export default function FormSelectComponent() {
  ...
  useEffect(() => {
    async function load() {
      const result = await getAllLocationsAction();
      setLocations(result);
    }
    load();
  }, []);
  ...
  return (
    // <Select.Root name={categoryId} items={categories}>
    ...
    <select name="location">
      {selectLocations.map(location => <option value={location.value} key={location.value}>{location.label}</option>)}
    </select>
  );
}
